...
- Added `-w` and `--workspace` as aliases for the shared Workspace option used by authentication, OIDC, and custom-domain discovery. `CLOUDSMITH_WORKSPACE` and `workspace` in `config.ini` are now the preferred environment variable and configuration key, with `CLOUDSMITH_ORG` and `org` retained as fallbacks. The existing `-o`, `--owner`, `--org`, `--organization`, and `--oidc-org` option spellings remain available for compatibility.
