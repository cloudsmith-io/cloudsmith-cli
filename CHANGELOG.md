...
- Added `-w` and `--workspace` as aliases for the shared Workspace option used by OIDC and custom-domain discovery. The existing `--org`, `--organization`, and `--oidc-org` spellings remain available for compatibility.
