...
_SKIP_AUTH_ARGS = ["--help", "-h", "--version", "-v", "help"]
_USER_SETTINGS_ARGS = ("-s", "--settings")
...
class MavenPlugin:
    ...
    def skip_provision_reason(self, args: list[str]) -> str | None:
        """A user-supplied settings file wins over injection.

        Prepending our own ``-s`` as well would silently shadow the user's
        file — Maven takes the first occurrence.
        """
        supplies_settings = any(
            arg in _USER_SETTINGS_ARGS or arg.startswith("--settings=") for arg in args
        )
        if supplies_settings:
            return (
                "the command supplies its own -s/--settings file; running mvn "
                "without Cloudsmith credential injection"
            )
        return None
