...
class DefaultDomain:
    ...
    """Format label as written in a ``[domains]`` config entry.

    Set only for a config-declared host whose label the CLI cannot resolve to a
    BackendKind, so the operator's own wording survives. Built-in hosts leave it
    unset and derive their label, which makes a label that contradicts its
    backend kind unrepresentable.
    """

    @property
    def format_label(self) -> str | None:
        """The package format this host serves, or None for no single format."""
        if self.declared_label:
            return self.declared_label
