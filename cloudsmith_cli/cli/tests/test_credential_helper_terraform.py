...
"""Tests for the `cloudsmith credential-helper terraform` runtime and CLI shim.
...
These tests pin the three ``get`` outcomes (token / empty-object / refusal) and
the rejection of ``store``/``forget`` and unknown verbs.
"""
