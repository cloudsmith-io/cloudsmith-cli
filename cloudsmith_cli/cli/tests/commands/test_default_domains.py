...
def test_builtin_labels_are_derived_from_backend_kind():
    """No built-in row declares a label, so none can contradict its kind."""
    for domain in BUILTIN_DOMAINS:
        assert domain.declared_label is None, domain.host
        assert domain.format_label == format_for_backend_kind(domain.backend_kind)
