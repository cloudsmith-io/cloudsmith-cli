...
# only runs under __main__ in the frozen bundle. Each must wrap its callable in
# sys.exit() so a failed command surfaces a non-zero exit code, not 0.
UNIMPORTABLE_ENTRYPOINTS = [
    (REPO_ROOT / "packaging" / "pyinstaller" / "entry.py", "sys.exit(main())"),
    (
        REPO_ROOT / "packaging" / "pyinstaller" / "terraform_entry.py",
        "sys.exit(run())",
    ),
]
...
@pytest.mark.parametrize(
    "path,expected", UNIMPORTABLE_ENTRYPOINTS, ids=lambda p: getattr(p, "name", p)
)
def test_unimportable_entrypoint_wraps_main_in_sys_exit(path, expected):
    if not path.is_file():
