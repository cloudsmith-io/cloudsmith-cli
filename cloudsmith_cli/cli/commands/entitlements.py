...
def print_entitlements(opts, data, page_info=None, show_list_info=True):
    ...
    for entitlement in sorted(data, key=itemgetter("name")):
        ent_updated_at = (
            entitlement["updated_at"].strftime("%Y-%m-%dT%H:%M:%S.%f%z").replace("+0000","Z")
            if entitlement["updated_at"]
            else None
        )
        ent_created_at = (
            entitlement["created_at"].strftime("%Y-%m-%dT%H:%M:%S.%f%z").replace("+0000","Z")
...
def print_entitlements_with_restrictions(
    ...
    for entitlement in sorted(data, key=itemgetter("name")):
        ...
        updated_at = (
            entitlement.get("updated_at").strftime("%Y-%m-%dT%H:%M:%S.%f%z").replace("+0000","Z")
            if entitlement["updated_at"]
            else ""
        )
        created_at = (
            entitlement.get("created_at").strftime("%Y-%m-%dT%H:%M:%S.%f%z").replace("+0000","Z")
            if entitlement["created_at"]
            else ""
        )
        ...
        limit_date_range_from = (
            entitlement.get("limit_date_range_from").strftime("%Y-%m-%dT%H:%M:%S.%f%z").replace("+0000","Z")
            if entitlement["limit_date_range_from"]
            else ""
        )
        limit_date_range_to = (
            entitlement.get("limit_date_range_to").strftime("%Y-%m-%dT%H:%M:%S.%f%z").replace("+0000","Z")
